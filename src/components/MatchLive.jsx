...
import { getPlayerLastName } from "@/lib/utils"; // Ripristinato il tuo import!
...
function RoleBadge({ role }) {
  return (
    <div className={`w-8 h-8 rounded-lg ${ROLE_COLORS[role] || 'bg-muted'} flex items-center justify-center font-display font-bold text-xs border flex-shrink-0`}>
      {role}
    </div>
  );
}
...
function SectionBlock({ label, homeEntries, awayEntries }) {
  ...
  // Stili base per i punteggi e nomi
  const scoreBaseClasses = "font-display text-base font-bold text-center rounded-lg px-2.5 py-1.5 border min-w-[64px]";
  ...
  const nameBaseClasses = "font-display text-sm uppercase tracking-wide truncate";
        ...
        {Array.from({ length: maxRows }).map((_, i) => {
          ...
          return (
            <div key={i} className="py-2.5 px-4">
              <div className="grid grid-cols-[auto,1fr,auto,20px,auto,1fr,auto] items-center gap-x-3.5 gap-y-1 sm:gap-y-0">
                ...
                {/* === SEPARATORE CENTRALE === */}
                <div className="flex justify-center text-center font-semibold text-muted-foreground text-xl">
                  -
                </div>
